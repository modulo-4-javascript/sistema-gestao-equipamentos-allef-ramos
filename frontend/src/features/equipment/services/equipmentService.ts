...
export const equipmentService = {
  ...
  async getEquipmentList(params: GetEquipmentListParams = {}) {
    const response = await axiosApi.get<PaginatedResult<Equipment>>(
      "/equipment",
      {
        params: {
          page: 1,
  ...
  async getEquipmentSummary() {
    const response =
      await axiosApi.get<EquipmentSummaryResponse>("/equipment/summary");
  ...
  async createEquipment(payload: CreateEquipmentPayload) {
    const response = await axiosApi.post<EquipmentDetail>(
      "/equipment",
      payload,
    );
